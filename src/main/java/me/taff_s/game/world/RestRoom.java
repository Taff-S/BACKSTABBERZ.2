...
public class RestRoom {
    ...
    public void enter(PlayerHandler handle1, PlayerHandler handle2) {
        ExecutorService inputExecutor = Executors.newFixedThreadPool(2);
        CountDownLatch choiceLatch = new CountDownLatch(2);

        try {
            Future<RestChoice> choice1Future = inputExecutor.submit(() -> getPlayerChoice(player1, handle1, choiceLatch));
            Future<RestChoice> choice2Future = inputExecutor.submit(() -> getPlayerChoice(player2, handle2, choiceLatch));
    ...
    private void sendRestMenu(PlayerHandler handler, Player player) {
        handler.getMessenger().sendMulti(
            "You both enter a quiet room. You can finally rest... or not.",
    ...
    private RestChoice getPlayerChoice(Player player, PlayerHandler handler, CountDownLatch choiceLatch) {
        sendRestMenu(handler, player);
        while (true) {
            String input = handler.getMessenger().prompt("What do you do?");
            ...
            switch (input.trim()) {
                case "1":
                    ...
                    awaitOtherChoice(choiceLatch);
                    handler.send("Waiting for the other player...");
                    return RestChoice.PEACEFUL;
                case "2":
                    player.setRestType(RestChoice.WARY);
                    choiceLatch.countDown();
                ...
                case "3":
                    try {
                        player.getInventory().manageInventory(handler, player);
                    } catch (IOException e) {
                        handler.send("Inventory error occurred.");
                    }
                    sendRestMenu(handler, player);
                    break;
                case "4":
    ...
    private void awaitOtherChoice(CountDownLatch choiceLatch) {
        try {
            choiceLatch.await();
    ...
    private void resolveChoices(RestChoice c1, Player p1, PlayerHandler ph1,
                                 RestChoice c2, Player p2, PlayerHandler ph2) {
        BackStabEncounter backStabEncounter = new BackStabEncounter(p1, p2);
        try {
            if (c1 == RestChoice.BACKSTAB && c2 == RestChoice.BACKSTAB) {
                ph1.send("You both stare each other down... then simultaneously lunge into battle!");
            ...
            System.err.println("IO error during backstab encounter: " + e.getMessage());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
